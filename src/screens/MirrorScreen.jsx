...
import { cancelDetection } from '../services/emotionDetector';

const MirrorScreen = () => {
  console.log('[MirrorScreen] BUILD CHECK v2', new Date().toISOString());
  ...
  useEffect(() => {
    if (detectionStatus === DETECTION_STATUS.COMPLETE && result && result.rasa) {
      ...
      const timer = setTimeout(() => {
        ...
        // Save AI vision data if available
        if (result.emotionNuance) {
          actions.setEmotionNuance(result.emotionNuance);
        }
        if (result.visionData) {
          actions.setVisionData(result.visionData);
        }
  ...
  const handleStreamReady = useCallback(
    (videoEl) => {
      ...
      console.log('[MirrorScreen] Starting (or restarting) detection...');
  ...
  const getStatusText = () => {
    ...
    if (detectionStatus === DETECTION_STATUS.VISION_REFINING) {
      return {
        main: visionInsight?.nuance
          ? `I see ${visionInsight.nuance}...`
          : 'The mirror is reading deeper...',
        sub: 'AI vision is refining your Rasa',
      };
    }
    if (detectionStatus === DETECTION_STATUS.COMPLETE) {
      return {
        main: result?.isVisionEnhanced
          ? 'Your Rasa has been deeply seen.'
          : 'Your Rasa has been revealed.',
        sub: result?.emotionNuance || '',
      };
    }
    ...
    >
      ...
      >
        ...
        <AnimatePresence mode="wait">
          ...
          {/* ===== CAMERA + DETECTING + VISION REFINING ===== */}
